...
class Culture_model {
  ...
  public function uploadtodb($nama, $post){
    $query = "INSERT INTO gallery_kerajinan VALUES ('0', :id_budaya, :foto)";
    $this->db->query($query);
  ...
  public function edit($data, $table){
    $query = "UPDATE ". $table .
             " SET id=:id, :nama_id, maps=:maps, artikel_id=:artikel_id, artikel_en=:artikel_en, nama_en=:nama_en, id_lokasi=:id_lokasi, rating=:rating 
              WHERE id=:id";
    $this->db->query($query);
